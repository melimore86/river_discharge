...
ui <- fluidPage(theme = shinytheme("yeti"),
   ...
   # Application title
   titlePanel("River Discharge Quantiles"),
...
server <- function(input, output) {
  ...
  plotInput <- reactive({
    ...
    ggplot(dis4) + 
      ...
      geom_line(aes_string(x= "id", y= input$year), size=1.1)+                      
      ylab(expression(paste('Mean Daily Discharge','  ',(ft^3 / sec)))) + 
      xlab('Day of the Year') + 
      scale_fill_discrete(guide = guide_legend(title = "Quantiles", reverse=FALSE),
